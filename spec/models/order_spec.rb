...
RSpec.describe Order, type: :model do
  ...
  describe 'state transitions' do
    ...
    context "when in state 'with_customer_data'" do
      ...
      it { expect(order).to_not transition_from(:with_customer_data).to(:waiting_for_payment).on_event(:fill_with_delivery_method).on(:state) }
      it { expect(order).to_not transition_from(:with_customer_data).to(:paid).on_event(:pay).on(:state) }
    end
    ...
    context "when in state 'with_delivery_method'" do
      ...
      it { expect(order).to transition_from(:with_delivery_method).to(:waiting_for_payment).on_event(:pay).on(:state) }
      it { expect(order).to_not transition_from(:with_delivery_method).to(:paid).on_event(:pay).on(:state) }
    end
